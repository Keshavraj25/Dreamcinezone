...
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002567856061'))  # Bin channel id (make sure bot is admin)
DREAMCINEZONE_MOVIE_UPDATE_CHANNEL = int(environ.get('DREAMCINEZONE_MOVIE_UPDATE_CHANNEL', '-1002608186529'))  # Notification of those who verify will be sent to your channel
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002567856061'))  # Premium logs channel id
...
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002409869914').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002409869914').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
...
DREAMCINEZONE_VERIFIED_LOG = int(environ.get('DREAMCINEZONE_VERIFIED_LOG', '-100'))  # Log channel id (make sure bot is admin)
...
DREAMCINEZONE_MOVIE_UPDATE_CHANNEL_LNK = environ.get('DREAMCINEZONE_MOVIE_UPDATE_CHANNEL_LNK', 'https://t.me/pikashow_7')
...
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '7045947967 6285713858').split()]
...
URL = "https://nervous-romonda-keshavraj-38c7a22e.koyeb.app/".format(FQDN) if ON_HEROKU or NO_PORT else "https://nervous-romonda-keshavraj-38c7a22e.koyeb.app/".format(FQDN, PORT)
